...
impl<R: BufRead> Records<R> {
    ...
    pub fn open(reader: R, forced: Option<Format>) -> io::Result<Self> {
        ...
        let records = match format {
            Format::Json => Self::with(format, Reader::Json(JsonRecords::new(source))),
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            match self.source.next_line() {
                ...
                Ok(Some(line)) => return Some(Ok(Record::text(line.text))),
                Ok(None) => return None,
...
impl<R: BufRead> Iterator for LogfmtRecords<R> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            self.started = true;
            return Some(match parse_logfmt(&line.text) {
                Some(value) => {
